...
#include <ignition/common4/ignition/common/Console.hh>
...
class ignition::gazebo::systems::PropellerPrivateData
{
  ...
  public: double thrustCoefficient = 1;
    /// \brief Drag coefficient relating the propeller angular velocity to the torque resisting its motion
...
/////////////////////////////////////////////////
Propeller::Propeller():
  dataPtr(std::make_unique<PropellerPrivateData>())
{
  // do nothing
}

/////////////////////////////////////////////////
void Propeller::Configure(
  ...
  EventManager &/*_eventMgr*/)
{
    ...
    {
        this->dataPtr->dragCoefficient = _sdf->Get<double>("drag_coefficient");
    }
    ...
    enableComponent<components::WorldAngularVelocity>(_ecm,this->dataPtr->linkEntity);
    enableComponent<components::WorldLinearVelocity>(_ecm,this->dataPtr->linkEntity);
  ...
  ignition::gazebo::EntityComponentManager &_ecm)
{
  ...
  auto angularVelocity =_ecm.Component<components::AngularVelocity>(this->dataPtr->linkEntity);
  ...
  double thrust = this->dataPtr->thrustCoefficient*angularVelocity->Data().X()*abs(angularVelocity->Data().X());    
  double torque = -this->dataPtr->dragCoefficient*angularVelocity->Data().X()*abs(angularVelocity->Data().X());
  ignmsg << angularVelocity->Data().X() << ", " << thrust << std::endl;
...
IGNITION_ADD_PLUGIN(
  Propeller, System,
  Propeller::ISystemConfigure,
  Propeller::ISystemPreUpdate)

IGNITION_ADD_PLUGIN_ALIAS(Propeller, "ignition::gazebo::systems::Propeller")
